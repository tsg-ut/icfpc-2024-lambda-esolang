...
type ski_fv_str = [ `I | `K | `S  | `Str of string | `Z | `Fv of int]

let pp_ski_fv_str_combinator = pp_combinator (fun fmt v ->
    match v with
    | `Fv x -> Format.fprintf fmt "a%d" x
    | `I | `K | `S | `Z | `Str (_ : string) as v-> pp_ski_str fmt v
  )


let enumerate_ski ~(size : int) ~(fvn : int) =
  let table = Array.make_matrix (size+1) (fvn+1) ([]:ski_fv_str combinator list) in

  table.(0).(0) <- List.map (fun v -> CVar v) [ `S; `K; `I; `Z ];
    ...
    for ai = 0 to fvn; do
      ...
      for j = 0 to i-1; do
        for aj = 0 to ai; do
          ...
          let tl = List.concat_map (fun l1 -> 
            List.map (fun l2 -> CApp(l1, lift_fv aj l2)) t2
          ) t1 in
  ...
  let res = Array.to_list table |> List.concat_map Array.to_list |> List.concat in
...
let _ =
  let combs = enumerate_ski ~size:4 ~fvn:2 in
  List.iter
    (fun c ->
...
let enumerate_partial_repr m ~(vn:int) =
  let rec aux m ~(vn:int) ~(off:int) =
    ...
    | CApp (m,n) as o ->
        if vn = 0 then [(o,fun m -> m)] else
        let rems = 
          List.init (vn+1) (fun i -> 
            let tm = aux m ~vn:i ~off in
            let tn = aux n ~vn:(vn-i) ~off:(off+i) in
            List.concat_map (fun (m,fm) -> List.concat_map (fun (n,fn) ->
              let base = [(CApp(m,n), fun o -> fn (fm o))] in
              if i = 1 && (vn-i) = 1 &&
                fm (CVar (`Fv off)) = fn (CVar (`Fv (off+1)))
              then
                (CApp(m,lift_fv (-1) n), fun o -> (fm o)) :: base
              else
                base
            ) tn) tm
          ) |> List.concat 
        in
          if vn = 1 then (CVar (`Fv off), fun n -> subst n (`Fv off) o) :: rems else rems
  in
  List.concat (List.init (vn+1) (fun vn -> aux m ~vn ~off:0))

let rec optimize_with_simpler_term m =
  let rec aux m =
      ...
      List.fold_left (fun acc (pm,f) ->
        ...
        | Some _ | None -> begin
            ...
            | Some h -> begin
                ...
                | Some (tm,l) ->
                    let ftm = f tm in
                    let ftl = String.length @@ Format.asprintf "%a" pp_ski_fv_str_combinator ftm in
                    if ftl < ml then
                      Some (ftm,tm,pm,l)
                    else acc
              end
    ...
    | Some (tm, h, pm, _) when tm <> m ->
        Format.eprintf "Reduce %a => %a by %a with pm %a\n"
          pp_ski_fv_str_combinator m
          pp_ski_fv_str_combinator tm
          pp_ski_fv_str_combinator h
          pp_ski_fv_str_combinator pm;
        tm
